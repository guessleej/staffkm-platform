...
| **計費漏錢點（v5.12 第二輪稽核）**：① Stripe topup webhook at-least-once 重送 → `add_credits` 無去重＝重複加值；② 旗艦模型在 `PROVIDER_DEFAULT_MODELS` 但 `MODEL_PRICING` 無價 → `calc_cost` 回 0；③ 預付 credits 只增不減（用量不扣）；④ trial `is_frozen` 標了無人 enforce | ① `add_credits` 用 reference 去重（FOR UPDATE 鎖內查 ledger + `credit_ledger` partial unique index）；② 新增模型**同 PR** 補 `MODEL_PRICING`/`MEDIA_PRICING`；③ `metering._consume_credits` 對 topup/trial plan 扣；④ `check_quota` 最前面擋 `is_frozen`。皆 v5.12 修 |
