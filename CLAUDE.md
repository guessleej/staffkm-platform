...
### 12. 模型清單：一律動態偵測，不寫死任何 seed（v5.11.x 起，v5.13 根除）
- ❌ **不要寫死任何 provider 的預設模型清單**。host/端點上沒有的就變「抓不到的幽靈模型」，且**啟動 seed 會一直種回來**（使用者刪了一重啟又復活）。
- **v5.13 已根除兩個寫死 seed 來源**（別再加回來）：
  - `model_pricing.PROVIDER_DEFAULT_MODELS` 整個 dict **已刪除**；`agent` 啟動 `pricing_seed.seed_model_pricing` 的「Part 0：per-provider INSERT model row」**已移除**，只留 `MODEL_PRICING`/`MEDIA_PRICING` 價格 UPDATE（只對既有 row 補價、不造模型）。
  - `auth/api/models._DEFAULT_MODELS_ON_CREATE` **全清空**（值全 `[]`、保留 key 只為過 `tests/integration/auth/test_model_fetch_coverage` 的「每型都有模型來源」守衛）。建供應商不再 seed 佔位。
  - `registry.recommended_models` 仍是純 UI 提示字（不寫進 DB、不是模型來源），低風險、保留。
- ✅ **唯一模型來源 = 即時動態偵測**（`auth/api/models.list_provider_models`，打開供應商清單就觸發、**多補少刪** self-heal）：
  - ollama → `/api/tags`（`_sync_ollama_models`）
  - OpenAI 相容（含 **Microsoft Foundry / Azure AI Foundry / LiteLLM / 類 OpenAI 自訂**）→ `/v1/models`（`_sync_openai_compat_models`）；`azure_openai` 只有傳統 `*.openai.azure.com` 才走 api-version 特殊路徑，其餘視為 OpenAI 相容。
  - model_type 靠關鍵字猜（embed→embedding / rerank→reranker / ocr·vision·-vl→vision / 其餘 llm）。
- 賣場 catalog 只暴露三類：**地端自架 + Microsoft Foundry + 類 OpenAI API（自訂）**（`registry.list_providers` 過濾；其餘大廠 `PROVIDER_REGISTRY` 仍在、只是不上下拉）。
...
| Python naive `datetime.utcnow()` 寫進 `timestamptz` 欄 → 偏移。**真機制（v5.12 實測釐清）：asyncpg 用「python 行程 TZ」（容器 `TZ` env）編碼 naive datetime，不是 postgres session TZ**。容器預設 TZ=UTC → 正確；客戶把容器設 `TZ=Asia/Taipei`(+8) 看 log → naive utcnow 偏 8h（觸發時間/API key·trial·驗證 token 到期全錯）| **根治：compose `x-db-env` 釘 `TZ: UTC`**（所有 python service 行程 UTC）。改 postgres session TZ **沒用**（已實測）。時間戳也可 server-side 算 `now()+make_interval`（webhook_outbox 這樣修）。日誌在地時間請在前端轉。`tests/integration/agent/test_webhook_outbox_integration` 守 |
...
| **對話答案「回應時發生錯誤」但 citations/檢索正常** → 極易誤判成 RAG / 程式 bug。真因常是 **host Ollama 沒在跑**：embedding 走 `embedder` 容器（仍活 → 檢索/citations 正常），但 `default.llm`（gemma4:e4b）在**主機 Ollama** `host.docker.internal:11434` → 沒跑時 agent LLM `Connection error`、只有「生成」這段失敗 | **先查 host Ollama，別先 debug RAG/程式**：`curl http://localhost:11434/api/tags`（或容器內 `host.docker.internal:11434`）。沒跑 → `OLLAMA_HOST=0.0.0.0:11434 ollama serve`（或開 Ollama.app；模型已下載**勿 pull**）。⚠ 重開機/睡眠後**不會自動起**。agent log 看 `application_agent_llm_stream_failed error=Connection error`。v5.12 此症狀誤導兩次才定位 |
| 對話端點按 conv_id 取資料但**漏 ownership check** → IDOR（知道/猜到 conv_id 就能讀別人對話內容）。`get_messages` 中過（`delete`/`share` 早有檢查、`get_messages` 漏了）| 任何「按 id 取 user-scoped 資源」的端點都要 `if user_id and conv.user_id not in (user_id, 'anonymous'): 403`（公開分享走獨立 /public + share_token 端點）。v5.12 整合測試挖出修；`tests/integration/chat` 守。`get_messages`（read）+ `stream_message`（write）+ `export`（read，v5.12 第二輪補）同類 IDOR 皆已補，整合測試各有斷言守。|
| **前端 markdown 渲染 XSS**：`marked` 預設**會輸出 raw HTML**（`<img onerror>`/`<script>`），LLM 回覆/RAG 命中/使用者輸入裸 `v-html` = XSS（對話主流程 + 公開匿名 PublicChatView/embed 最廣）| 任何 markdown→`v-html` 一律走 `apps/web/src/utils/markdown.ts` 的 `renderMarkdown()`（marked→`DOMPurify.sanitize`）；把使用者字串拼進 HTML（如 highlight `<mark>`）先 `escapeHtml()`。不要自己 `marked.parse`。v5.12 第二輪加 dompurify dep + 集中 util |
| **計費漏錢點（v5.12 第二輪稽核）**：① Stripe topup webhook at-least-once 重送 → `add_credits` 無去重＝重複加值；② 動態偵測到的新模型（如 Foundry/自訂端點）若 `MODEL_PRICING` 無價 → `calc_cost` 回 0（原文舉的 `PROVIDER_DEFAULT_MODELS` 已於 v5.13 刪除，模型改動態偵測，但「新模型要補價」原則不變）；③ 預付 credits 只增不減（用量不扣）；④ trial `is_frozen` 標了無人 enforce | ① `add_credits` 用 reference 去重（FOR UPDATE 鎖內查 ledger + `credit_ledger` partial unique index）；② 新增模型**同 PR** 補 `MODEL_PRICING`/`MEDIA_PRICING`；③ `metering._consume_credits` 對 topup/trial plan 扣；④ `check_quota` 最前面擋 `is_frozen`。皆 v5.12 修 |
| **新增依賴授權陷阱（要賣錢）**：`trafilatura` <1.8.0 是 **GPLv3**（傳染性），≥1.8.0 才 Apache-2.0；預設 LLM `gemma4:e4b` 走 **Gemma 授權**（非 Apache/MIT，有 Prohibited Use Policy）| `trafilatura` 釘 ≥1.8.0、release checklist grep 防降版；Gemma 須向客戶**揭露授權**（權重未打包、runtime `ollama pull`，暴露面較低）。前端 `^`/後端 pyproject `>=` 浮動版本 → 交付前 `--frozen-lockfile`/`uv lock`。`httpx` 各 service 版本待統一（knowledge 0.27.2 vs 其餘 0.28.1）|
| **純 HTTP 部署下對話送出「靜默炸」**：`crypto.randomUUID()` 是**安全環境限定**（HTTPS / localhost）API，純 HTTP origin（如地端 `http://192.168.11.50`）下是 `undefined`，呼叫即 `TypeError`。對話送出 `onSubmit` 在 `addUserMessage`/`startAssistantMessage` 一呼叫就炸 → 訊息沒送、輸入框沒清、**零回饋**（極易誤判成「設了模型卻沒反應」/ 模型 / 後端問題；DB `messages` 表 0 筆是鐵證——根本沒送出）。v5.13 地端 HTTP 部署實戰挖出，繞了一大圈才靠瀏覽器 Console 紅字 `crypto.randomUUID is not a function` 定位 | 前端產 UUID 一律走 `apps/web/src/utils/uuid.ts` 的 `uuid()`（`randomUUID`→`getRandomValues`(HTTP 可用)→`Math.random` 三層退路），**不要裸呼 `crypto.randomUUID()`**。更廣：純 HTTP 還會卡 clipboard / `getUserMedia` / Service Worker 等 secure-context API → 地端正式環境建議上 HTTPS（Caddy auto-TLS / 自簽）。診斷「對話無反應」先看**瀏覽器 Console**，不只後端 log |
